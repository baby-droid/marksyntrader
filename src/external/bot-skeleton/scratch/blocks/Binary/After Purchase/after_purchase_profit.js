...
window.Blockly.Blocks.after_purchase_profit = {
    init() {
    ...
    definition() {
        return {
            ...
            args0: [
                ...
                {
                    type: 'field_label',
                    text: localize('4. Restart trading conditions (with profit tracking)'),
            ...
            tooltip: localize('Get the last trade information and result, track profit, then trade again.'),
            category: window.Blockly.Categories.After_Purchase,
        };
    },
    meta() {
        return {
            display_name: localize('Restart trading conditions (with profit tracking)'),
            description: localize(
                'Here you can run profit-tracking actions and decide if your bot should continue trading.'
            ),
        };
    },
    onchange(event) {
...
window.Blockly.JavaScript.javascriptGenerator.forBlock.after_purchase_profit = block => {
    const stack = window.Blockly.JavaScript.javascriptGenerator.statementToCode(block, 'PROFIT_ACTIONS');
    const code = `
