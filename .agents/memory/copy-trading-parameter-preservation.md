...
Bot Builder's normal purchase is observed through `bot.contract`, but direct side purchases used by execution-speed fan-out do not open an `OpenContract` subscription. Those purchases must publish their pre-buy and post-buy signals inside the purchase function.
