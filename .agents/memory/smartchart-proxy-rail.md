...
**How to apply:** Keep native SmartChart controls mounted inside a dedicated hidden proxy wrapper, retain their DOM for programmatic clicks, hard-constrain the visible rail buttons so SmartChart flex rules cannot stretch them, let SmartChart choose its drawing overlay position instead of forcing stale coordinates, proxy each rail action through the nearest chart wrapper, and style `.smartcharts-quill-portal` rather than rebuilding indicator/drawing functionality. Keep the panel small and floating, not full-height or centered over the chart.